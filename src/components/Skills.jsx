...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-20 relative bg-[#070a0f]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center text-center space-y-3 mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
            Skill <span className="text-[#00ff9d]">Matrix</span>
          </h2>
        ...
        <div className="space-y-6 mb-12">
          ...
          <div className="max-w-md mx-auto relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-900/90 border border-slate-800 text-slate-100 text-xs sm:text-sm font-mono focus:outline-none focus:border-[#00ff9d]/60 focus:ring-1 focus:ring-[#00ff9d]/40 transition-all placeholder:text-slate-500"
            />
            {searchQuery && (
              <button
        ...
        <div className="space-y-10">
          {filteredCategories.length === 0 ? (
            <div className="text-center py-12 glass-card rounded-2xl border border-slate-800">
              <p className="text-slate-400 font-mono text-sm">No skills found matching "{searchQuery}"</p>
              <button
                onClick={() => { setSearchQuery(''); setActiveCategory('All'); }}
                className="mt-3 text-xs font-mono text-[#00ff9d] underline"
              >
                Reset filters
              </button>
            </div>
                ...
                <div key={idx} className="glass-card rounded-2xl p-6 border border-slate-800 space-y-6">
                  ...
                  <div className="flex items-center justify-between pb-4 border-b border-slate-800">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 font-sans">{group.description}</p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    {group.skills.map((skill, sIdx) => (
                      <div
                        key={sIdx}
                        className="p-3.5 rounded-xl bg-slate-900/70 border border-slate-800/80 hover:border-[#00ff9d]/40 transition-all hover:bg-slate-900 flex items-center justify-between group"
                      >
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-950 text-emerald-400 border border-emerald-950">
                              {skill.tag}
                            </span>
                          </div>
                        </div>
